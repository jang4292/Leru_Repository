...
import com.bpm202.SensorProject.Account.JoinDetailsActivity;
...
public class DBUser extends BaseDB {
    ...
    public ArrayList<PersonalInfoObj> selectTokenDB(String token) {
        String selection = String.format("%s=?", TABLE_USER.COLUMN_TOKEN);
    ...
    public Boolean insertDB(Object obj) {
        String token = new AppPreferences(mCon).getStringPref(AppPreferences.KEY_TOKEN);
        PersonalInfoObj vo = (PersonalInfoObj) obj;
        ContentValues cv = new ContentValues();
        cv.put(TABLE_USER.COLUMN_NAME, vo.getNickname());
        cv.put(TABLE_USER.COLUMN_TOKEN, token);
